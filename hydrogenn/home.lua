...
if ccc and kkk and activity.getSharedData("开源提示")==nil then
  ...
  双按钮对话框("提示","本软件已开源 请问是否跳转开源页面?","我知道了","跳转开源地址",function()
    关闭对话框(an) end,function()
    关闭对话框(an) 浏览器打开("https://github.com/huajiqaq/Hydrogenn/")
  end)
end
...
function 主页刷新(页数)
  ...
  function 主页推荐刷新(result)
    ...
    local url="https://api.zhihu.com/feed-root/section/"..tointeger(result).."?channelStyle=0"
...
function 日报刷新()
  ...
  for i=1,2 do
    if i==2 then
      ...
      cal.add(Calendar.DATE,tonumber(aabba));
      d=cal.getTime();
  ...
  if code==200 then
    ...
    if updateversioncode > versionCode and activity.getSharedData("version")~=updateversioncode then
      ...
      myupdatedialog.getButton(myupdatedialog.BUTTON_POSITIVE).onClick=function()
        下载文件对话框("下载安装包中",updateurl,"Hydrogenn.apk",false)
      end
