...
local function setattribute(root,view,params,k,v,ids)
  if k=="layout_x" then
   ...
   elseif k=="url" then
    view.loadUrl(url)
   elseif k=="src" then
    if v==util.src("arrow_back") then
      view.contentDescription="返回"
     end
    if v:find("^%?") then
   ...
   elseif k=="include" then--dingyi增加
    if type(v)~="table" then
      error(string.format("loadlayout error: include value Must be a table, checked import layout.",0))
     else
      view.addView(loadlayout(v.layout))
    end
...
local function loadlayout(t,root,group)
  ...
  if t.style then
    if type(t.style)=="number" then--dalao
      style=t.style
     else
  ...
  if t.layout==nil then --mythoi增加
    ...
    isxml=(t.layout):find("R%.layout%.[%w_%s]+$",1,false)--mythoi增加
    if isxml then --mythoi增加
      view=LayoutInflater.from(context).inflate(loadstring("return "..t.layout)(), nil) --mythoi增加
     else --mythoi增加
      view=loadlayout(t.layout) --mythoi增加
    end --mythoi增加
  end --mythoi增加

  local params=ViewGroup.LayoutParams(checkValue(t.layout_width) or -2,checkValue(t.layout_height) or -2) --设置layout属性
  if group then
    local a,b=pcall(function() return group.LayoutParams(params) end)
    if a==true then
      params=b
     else
